...
void setupBLE() {
    ...
    const char* schemaJson = R"json([
        ...
        { "key": "server", "label": "IP del Servidor", "type": "string", "required": true },
        { "key": "port", "label": "Puerto", "type": "number", "required": true, "default": 1234 }
    ])json";
...
bool loadCredentials() {
    ...
    if (savedSsid.length() > 0 && savedServer.length() > 0) {
        ...
        // Remove port from host if user accidentally added it (e.g. 192.168.1.1:1234)
    ...
    if (spotify.getStatus(&spotifyHttp, serverUrl, status)) {
      if (status.is_playing) {
        ...
        if (spotifyMode) {
           ...
           spotifyMode = false;
           lastImageHash = "";
           lastGraphHour = -1; // Force graph update
        }
      }
    }
  }
...
void setup() {
  ...
  if (hasCreds) {
    ...
    Serial.println("No hay credenciales. Esperando configuración por BLE...");
    datos = (Data*) malloc(sizeof(Data) * 3); // Reserva mínima para evitar crash
  }
